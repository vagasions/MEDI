...
### 3.11 데이터 준비·백테스트 (비계획 정지 예방 워크플로)

- **신호 추천** (`ontology.signalRequirements`): 고장모드 라이브러리의 증상을 역추적해 클래스별 필요
  신호를 필수/권장으로 자동 도출. 파생지표 증상은 `DERIVED_INPUTS`로 실제 태그 role까지 풀어냄.
  각 신호가 어떤 고장모드 검출에 쓰이는지 근거 동봉 — 계측 수집 요청의 근거 자료.
- **태그 브라우저 + 사용자 조합** (`addCustomAsset`): 히스토리안 태그 검색 → ISA-5.1 자동 분류·자동
  role 매핑 → 사용자 정의 자산(A-USER 구역) — 기존 설비와 동일 파이프라인으로 진단.
- **백테스트** (`js/analytics/backtest.js`): 시간 전진 재생, 각 시점에서 과거 데이터만 사용(누수 없음).
  캘리브레이션 구간은 기록 앞 40%(48h~7일 클램프) 고정 — 학습/평가 분리. m-of-n 알람엔진 그대로 재생.
  실제 고장 시각 대비 리드타임 산출. 검증: 베어링 시나리오 리드타임 130h, 정상 14일 데이터 경고 0.
  주의: 시뮬레이터 부하 주기를 절대시간(60h/29h)으로 수정 — 히스토리 길이에 비례하던 아티팩트 제거
  (백테스트 캘리브레이션이 운전 영역을 항상 커버하도록).
- **배포**: `config.js`(window.MEDI_CONFIG)로 게이트웨이 주소 등 공통 하드코딩 — 사내 웹서버/공유폴더
  1회 배치로 전 PC 접속. 브라우저 알림(Notification API) 옵트인 — 긴급/높음 알람.

### 3.12 오토파일럿 — 자동 연결/트러블슈팅 (`backend/autopilot.py` + 프론트 autoConnect)

- 포트 탐지(asyncio 소켓, 신규 의존성 0) → REST /auth-info 검증(보안/TLS) → 커넥터 우선순위 자동 채택.
- TLS 자가서명: 우회 재시도로 연결 유지 + 인증서 등록 요청문. OAuth 활성: 토큰 요청문.
- 타임존 자동 보정: `detect_ts_offset_ms` — 최신 타임스탬프와 UTC 차이가 30분 배수(±6분 허용,
  10분 이내 정상, 26h 초과는 stale)면 오프셋으로 판정, read 응답에 적용. 시뮬레이터는 검사 제외
  (요청 구간에 맞춰 생성하므로 오검출). 단위검증: 정상 0 / +9h(KST) / −9h / 30분대(+5.5h) / stale 0.
- 읽기 자동 재시도: 게이트웨이(1s/2s 백오프 3회) + 웹앱 datasource(동일) 이중.
- 웹앱 autoConnect: 주소 후보(입력값→config.js→localhost) 자동 시도·전환·저장, 로드 실패 시
  세션당 1회 자동 실행, 마지막 정상 데이터 유지 배너. E2E 검증: 틀린 주소 → 자동 전환 → 데이터 흐름.
- 사람 조치 요청문 생성기: 방화벽/OAuth/읽기권한/TLS — 수신처·목적·근거 포함 복사용 텍스트.
...
- 설비 내 연관 분석: `mv.corrShiftPairs` — 베이스라인 vs 최근 상관행렬 차이 상위 쌍 + 산점도
  (정상/최근 색 분리) — SPE 경보를 신호쌍 단위로 해석.
